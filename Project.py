#Food_court_name:Type_of_food(Food_stall_name,price)
...
def display_map():
    #get the NTU map picture from your laptop
    introScreenImage = pygame.image.load("C:Users/Lenovo/Tugas ITC/NTU Campus.png")
    screen = pygame.display.set_mode((900,700))
...
#get the user location by clicking on map then the user can close by click quit button on the right corner of the pygame and continue the program
while running:
    event = pygame.event.poll()
    if event.type == pygame.QUIT:
        running = 0
    elif event.type == pygame.MOUSEBUTTONDOWN and event.button == LEFT:
        initial_location=("%d %d" % event.pos)
        initial_location = tuple(map(int, initial_location.split(' ')))
...
#make a new canteen dictionary which already sorted based on user money
def check_money():
    dict_1={}
    money=-1
    while money<0:
